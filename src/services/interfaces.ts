...
export interface OpenidForCredentialIssuingInterface {
	...
	sendAuthorizationResponse(req: Request, res: Response, bindedUserSessionId: string, selectedCredentialIdList?: string[]): Promise<void>;
...
export interface OpenidForPresentationsReceivingInterface {
	...
	authorizationRequestHandler(req: Request, res: Response, userSessionIdToBindWith: string): Promise<void>;
	 ...
	 * @param res 
	 */
	responseHandler(req: Request, res: Response): Promise<{ verifierStateId: string, bindedUserSessionId: string }>;
